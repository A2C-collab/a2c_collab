...
class LLMRunner:
    def __init__(
        ...
        self.env.reset()
        self.robots = robots
        self.robot_agent_names = list(robots.keys())
    ...
    def one_run(self, run_id: int = 0, start_step: int = 0, skip_reset = False, prev_llm_plans = [], prev_response = None, prev_actions = None):
        ...
        llm_timing_path = os.environ.get("TIMING_LOG", "timing_metrics.log")
        self.prompter.save_llm_timing(llm_timing_path)
        self.prompter.reset_llm_timer()
...
def main(args):
    ...
    runner = LLMRunner(
        ...
        llm_output_mode=args.output_mode,
